...
- Server 使用 PostgreSQL Command Journal，以 `(session_id, command_id)` 为唯一幂等身份；相同请求摘要恢复或返回已有结果，不同摘要明确冲突。RulePlugin 请求必须在执行前持久化，因其 deterministic + no_io 可用完全相同请求重放；已 dispatched 但无结果的模型调用保持 ambiguous/blocked，禁止自动重调；Provider 已返回完整响应且命中闭合确定失败集合时写入内部 `failed_definite`（非公共协议字段），命令以原始失败码失败，同一 `request_id` 仅允许下一次显式命令重试再 dispatch；可信部署面可用 `kernel.modelInvocations.voidAmbiguous`（deployment-api only）把 `dispatched_ambiguous` 裁决为 `model.operator.adjudicated_void` 的 `failed_definite`，禁止 Client Bridge 暴露；`apply_packet` 只用同一 packet ID 幂等重试；编排从最后持久阶段继续且不得写入 WorldState。
