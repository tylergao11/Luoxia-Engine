...
| **staging**（`none` / `prefab_bind.stage_index + entry_mode_index` / `improv` 草稿） | 以锁定舞台目录校验参与人数、玩家在场与所选 entry mode 的 `npc_participation`，再物化为 `StageCatalogRef + entry_mode_id` 或 `MaterializedImprovStage`；封存 `SealedEventResult.staging` 与卡 `staging_kind` |
...
**Agency / 证据不双写**：`character.dialogue` **必须**输出本轮承诺增量 `commitments` 数组（无新增或实质变更时 `[]`）；复述、确认仍有效旧约不产生第二条事实，Server 在唯一承诺物化入口也会按语义、主体、立场与条款去掉有效旧约的完全重复项。Server 写入 turn 后，Provider 对 `character_mind` 回合**始终投影** `agency_commitments`（含 `[]`）给 `director.dialogue_events`——这是**只读上下文**，不是让事件模型再抄一遍。`commitment_evidence` **不是** EventCard 模型草稿字段；物化时 Server 只从本轮最新 Character responder turn 的 `agency_commitments` 确定性装配证据引用，历史承诺不能授权新舞台。`prefab_bind` 的有效 gate 完全由所选 entry mode 的 `npc_participation` 与该 commitment 推导：`unilateral` 固定无 gate，`commitment_required` 用承诺原始 subjects / semantic_intent / terms、承诺者和全部封存 outcomes 装配唯一 gate；Director 对 prefab 写入的 gate 壳不参与事实。非 prefab outcome 仍可使用模型 gate 壳，但证据始终由 Server 绑定。零承诺时有效 gates 为 `[]` 并清除悬空 `requires_agency_gate_index`。受保护 outcome 在封存时写入 `agency.commitment_valid` 前置条件。

**Stage / 唯一编排决定**：EventCard 草稿必填 `staging`（`none` / `prefab_bind` / `improv`），它是一次对话交换中“是否进入哪座舞台、从哪个入口进入”的唯一决定；Character reply 只拥有人物反应与 commitment，不拥有舞台路由，也不能裁定胜负。`gameplay.stages` 是闭合舞台目录：stage 拥有玩法、参与人数与运行态，`entry_modes[]` 拥有意图边界、示例与 `npc_participation`。因此强制交锋和正式邀战可以进入同一个对决玩法，前者 `unilateral`，后者 `commitment_required`，不再建立相互冲突的两个舞台。Director 只选择 `stage_index + entry_mode_index + participants`；Server 用锁定目录物化 `StageCatalogRef + entry_mode_id` 并装配 gate。若封存含 `stage.open`，仅在 **trigger 成功后** 打开；过程与胜负由 `stage_outcome.resolve` 裁定，不由聊天台词预封。日切离 player 时同包 expire 未开启卡；下一日 player 同包 `event_budget.open`。
...
`world_view.actors[*].name` 只从锁定 `EntityState.name` 投影，并随 actor 索引提供给 Director；它是生成文案所需的权威称呼，不是第二份人物目录。Provider 可省略实体 UUID，但不得省略姓名后让模型猜测人物身份。
...
**上下文裁剪（成本核心）**：`director.dialogue_events` 与 `director.goal_plan` 的正式 `world_view` 只投影对话相关子图——实体参与者、一度关系邻居、相关 facts（knowledge / visibility.actor_ids / claim 引用命中）、world machines、必填 `stages[]` 与必填 `event_budget`——禁止默认全图。`director.daily_settlement` 保留较宽视图（全部 `active` 实体 ∪ 结算窗口 `objective_traces` 主体，省略未入 traces 的 retired 噪声）且**省略** `stages` / `event_budget`（日结不写 EventCard staging；预算仅在进入 player 时打开）；空 `objective_components` 与 `objective_traces.visibility` 仍由 Provider 投影省略。`character.dialogue` 的近期原文窗口与正式记忆召回数量由 Deployment 的唯一 `CharacterDialogueContextPolicy` 显式配置；Engine 只保留协议安全上限 **24 turn**。近期原文之外、但仍有效且属于当前 CharacterMind 与当前对话对象的 transcript commitments，以 `prior_active_commitments` 从正式 DialogueRecord 只读召回；这包含同一双方已经关闭的相关对话，近期承诺仍只存在于原 turn，不重复复制。长期信息只召回正式 `MemoryRecord`，按 `salience` 排序；不新增摘要模型或第二份记忆。`director.dialogue_events` 仍只观察本次已经完成的 **2 turn** human→responder exchange，确保旧承诺不能再次授权本回合 `staging`。`director.dialogue_events` 仅在 `remaining > 0` 时调用且必产 1 张卡。DeepSeek 等 Provider 保持稳定 system 前缀 + 动态 JSON 仅在 user；invocation log 继续记录 `promptCharCount` / `modelInputByteCount`。
