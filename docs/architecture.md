...
ContentBundle Loader（Schema + digest + 语义门禁）
  ...
  → 唯一 StageModule Registry
       · module_id 唯一、manifest 内 scene_id 唯一
       · StageModuleLock 仅从 manifest 四字段派生并再经 common.v1 Schema 校验
       · depends_on_module_ids 必须命中已注册 module_id、禁止自依赖、整体无环
  → 校验 required content_pack；精确解析 required stage_module DependencyLock 为 root
  → 对指向 required stage_module 的 StageRef 校验 scene_id
       （bundle.worlds[].default_stage、bundle.presentation.bindings[].stage）
  → planRequiredModules：required roots 的传递依赖闭包，依赖优先、稳定排序
  → 收集 required rule_plugin 依赖身份（不创建 ABI）
  → required asset_provider 仍明确 unsupported
  → 构造与该 Catalog 一致的 RuntimeExecutionKernel
```

Registry 只证明依赖与场景合同可用；`requiredStageModules` 激活计划只负责验证与依赖优先排序，不加载、不执行 `entrypoint` 或任何 StageModule 制品，也不写入 ContentBundle / WorldState / 数据库。optional stage_module 与未被 required root 引用的已注册 manifest 不进入计划。entrypoint 的实际加载属于具体 Client Runtime Host。StageModule 制品 ABI 与 GDJS / Unity / UE5 实现仍未决定或实现。Kernel 与 World Core 不依赖 `apps/gdjs-host`，也不加载 StageModule 代码。返回 Kernel、已验证 `(pack_id, bundle_digest)` 身份列表、同一个只读 StageModule Registry，以及 required 激活计划（元素为 Registry 原注册对象）；不复制 content 文档或 manifests 成第二份可变数组。禁止默认目录、内容扫描、环境变量猜路径、内置示例包、示例插件或默认 StageModule。引擎仓库不承载具体世界内容、具体 RulePlugin 实现或具体 StageModule 制品。
...
服务端 Content Activation 已建立引擎中立的 StageModule manifest Registry：组合根显式传入不可信 manifests，经 `client-bridge.v1` 校验后锁定 `module_id`、`StageModuleLock`、scene 索引与 `depends_on` DAG。该 Registry 只服务依赖与场景合同门禁，不是完整 Stage Runtime；不读取目录、不动态 import、不执行制品。真实加载与运行仍由可替换的 Client Runtime Host 完成（首版适配器骨架为 `apps/gdjs-host`）。
...
首版纯原画表现同样经 Client Runtime Host 呈现：场景原画、人物立绘、表情、转场、镜头、微动、天气、粒子与音效。未来新增可行走角色或复杂舞台只需新增 StageModule，不修改 World Core 权威合同。Client Runtime 适配器可替换（首版为 GDJS Host 骨架；未来可为 Unity / UE5），不改变上述权威边界。StageModule 制品 ABI 与具体客户端运行时接入仍未实现。
...
模型协议只使用严格、版本化 JSON Schema，不使用语义 DSL，不向模型暴露客户端运行时字典、WorldState 写句柄或 EffectOp。运行时只有两个模型层级：
